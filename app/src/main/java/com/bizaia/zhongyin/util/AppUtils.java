...
public class AppUtils {
    ...
    public static  void addAlarm(long time,String content,Context context) {
        try {
            String[] projection = new String[]{"_id", "name"};
            Uri calendars = Uri.parse(String.valueOf(CalendarContract.Calendars.CONTENT_URI));
            ...
            event.put("title", content);
            event.put("description", "Event Desc");
            event.put("eventLocation", "Event Location");
            event.put("dtstart", time);
            event.put("dtend", (time + 3600000));
            //设置一个全天事件的条目
            ...
            context.getContentResolver().insert(Uri.parse(String.valueOf(CalendarContract.Reminders.CONTENT_URI)), values);
        }catch (Exception e){
            Log.e(TAG, "addAlarm: --------------------->error---->"+e.getMessage() );
        }
        Log.e(TAG, "addAlarm: --------------->set  alarm  success" );
    }

    public static void deleteAlarm(Context context,String title){
        Cursor eventCursor = context.getContentResolver().query(Uri.parse(String.valueOf(CalendarContract.Events.CONTENT_URI)), null, null, null, null);
        try {
            ...
            if (eventCursor.getCount() > 0) {
                ...
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(title) && title.equals(eventTitle)) {
                        ...
                        Uri deleteUri = ContentUris.withAppendedId(Uri.parse(String.valueOf(CalendarContract.Events.CONTENT_URI)), id);
